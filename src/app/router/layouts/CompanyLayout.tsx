...
export function CompanyLayout() {
  ...
  const { data: notifData } = useQuery<{ unread: number }>({
    queryKey: ['company', 'notifications', 'unread'],
    queryFn: async () => {
      const { data } = await apiClient.get('/company/notifications/unread-count')
      ...
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-border bg-white px-4 sm:px-6">
          ...
          <div className="flex items-center gap-3">
            {/* Notifications */}
            <Link to={ROUTES.COMPANY_NOTIFICATIONS} className="relative p-2 rounded-lg hover:bg-gray-100">
